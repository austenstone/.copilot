...
## Transitive contract review

Choosing a reusable workflow creates a versioned job contract. Review every edge, not only the top-level caller:

| Contract | What must remain compatible |
|---|---|
| Inputs | Name, type, required/default behavior, and the value forwarded at each nested call |
| Secrets | Explicit mapping or `inherit` on each direct edge; environment secrets are resolved where the callee job declares its environment |
| Outputs | Step-to-job and job-to-workflow mappings, then every caller expression consuming them |
| Permissions | Workflow/job scopes and any downgrade across nested calls; never assume a callee can recover a permission removed upstream |
| Environment | String versus object form, approvals, protection rules, deployment identity, and URL |
| Concurrency | String versus object form, group expression, cancellation behavior, and collisions between caller and callee |
| Runner | Labels, runner-group access, architecture, network reachability, capacity, and billing owner |
| Reference | Each remote `@ref` independently; a SHA-pinned caller can still reach a tag-pinned nested callee |

If any required consumer or callee is inaccessible, the decision is conditional until its contract is checked.
...
| `workflow_run` | Chain from one workflow's result to another trust boundary | Accidentally deploying artifacts rebuilt from a different ref. `workflow_run` can access secrets and write tokens even if the previous workflow was unprivileged. Fetch the live chaining limit before designing a multi-stage chain. Source: [events that trigger workflows](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#workflow_run). |
